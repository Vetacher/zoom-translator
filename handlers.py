...
from app.database import SessionLocal, create_meeting_session, get_active_sessions, update_session_status, get_or_create_user_settings
...
async def new_session_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработчик команды /new - начало создания новой сессии"""
...
async def handle_zoom_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка полученной ссылки на Zoom"""
    ...
    # Создаём клавиатуру с языками (исходный язык)
    keyboard = []
    for code, name in list(languages.items())[:8]:  # Первые 8 языков
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('source_'):
        ...
        # Показываем языки для перевода
        languages = AzureSpeechTranslator.get_supported_languages()
        keyboard = []
        for code, name in list(languages.items())[:8]:
            if code != source_lang:  # Исключаем исходный язык
    ...
    # Обработка выбора времени
        ...
        if data == 'time_now':
            # Создаём сессию и запускаем сейчас
...
async def create_and_start_session(query, context, scheduled_time=None):
    """Создаёт и запускает сессию перевода"""
    ...
    try:
        ...
        if scheduled_time:
            ...
            status_emoji = "🟢"
            # Здесь будет запуск перевода
            # TODO: Implement actual connection to Zoom
